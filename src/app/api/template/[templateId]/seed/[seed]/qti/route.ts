...
export async function GET(
	...
	if (!templateIdResult.success) {
		logger.error(
			"template seed qti redirect route received invalid template id",
			{
	...
	if (!seedResult.success) {
		logger.error("template seed qti redirect route received invalid seed", {
			templateId,
			seed: params.seed
		})
	...
	if (latestAttempt === null) {
		logger.error(
			"template seed qti redirect route found no validated attempts",
			{ templateId }
		)
		return NextResponse.json(
	...
	const redirectUrl = new URL(
		`/api/template/${templateId}/attempt/${latestAttempt}/seed/${seed}/qti`,
		request.url
	)
